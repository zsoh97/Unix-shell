...
public class ArgumentResolver {
    ...
    private boolean isNotEmpty(Stack<Character> list) {
        return !list.isEmpty();
    }
    ...
    private String evaluateSubCommand(String commandString) throws AbstractApplicationException, ShellException {
        ...
        // remove new lines
        String result = removeLastNewLine(outputStream.toString());
        return result.replace(STRING_NEWLINE, " ");

    }

    private String removeLastNewLine(String str) {
        if (str.endsWith(STRING_NEWLINE)) {
            return str.substring(0, str.length()-STRING_NEWLINE.length());
        }
        return str;
    }
